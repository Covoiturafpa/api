...
public interface RideRepository extends CrudRepository<Ride, Integer>, RideRepositoryCustom {
    ...
    @Query("DELETE FROM RidePassenger rp WHERE rp.ride.id = :idRide AND rp.person.id = :idPerson")
    public int rejectReservationOfRideByPerson(@Param ("idRide") int idRide, @Param("idPerson") int idPerson);
}
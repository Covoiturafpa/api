...
public interface RideRepository extends CrudRepository<Ride, Integer> {
    ...
    @Query("SELECT ride FROM Ride ride JOIN RecurringRide WHERE ((:destination.city IN (SELECT d.city FROM Destination d WHERE d.city IS NOT NULL)) OR (distance(:destination.latitude, :destination.longitude, ride.destination.latitude, ride.destination.longitude))) AND ")
    ...
    @Query("SELECT ride FROM Ride ride JOIN OneTimeRide WHERE ((:destination.city IN (SELECT d.city FROM Destination d WHERE d.city IS NOT NULL)) OR (distance(:destination.latitude, :destination.longitude, ride.destination.latitude, ride.destination.longitude))) AND ")
    ...
    @Query("SELECT rde FROM Person usr JOIN RidePassenger rp ON usr.id = rp.id.idUser JOIN Ride rde ON rp.id.idRide = rde.id WHERE rp.id.idUser = :id")
